...
**Q: 提示词条目支持哪些角色？**
A: `system`（系统提示）、`user`（用户消息）、`assistant`（助手消息）。一般规则提示用 `system` 最稳。
...
**Q: 温度建议设多少？**
A: 抽取/分析类工作流建议中低温度（0.3-0.8），创意或扩写类可适当提高（1.0-1.5）。

**Q: 频率惩罚和存在惩罚的区别？**
A: 
- **频率惩罚**：降低模型重复使用同一词汇的倾向，值越高越抑制重复
- **存在惩罚**：鼓励引入新词，适当提高可减少原地重复
...
**Q: 注入位置填什么？**
A: 常见值是 `before_character_definition`（角色卡定义前）。这决定了条目内容在 AI 提示词链中的插入位置。
