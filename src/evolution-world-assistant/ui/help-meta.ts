export type TabKey = 'overview' | 'global' | 'flows' | 'debug';
...
const FIELD_HELP_LIST: FieldHelpMeta[] = [
  ...
  },
  {
    key: 'dispatch_mode',
    label: '调度模式',
    shortHelp: '并行会同时请求所有流，串行会按顺序执行。',
    detailHelp: '串行模式下后续流可读取前序流结果；并行模式更快，但冲突由优先级和顺序合并。',
  },
  {
    key: 'total_timeout_ms',
    label: '总超时',
    shortHelp: '整轮工作流的最大耗时上限（毫秒）。',
    detailHelp: '超过该时间视为失败并中止本轮发送。建议不小于所有启用流超时的最大值。',
  },
  {
    key: 'gate_ttl_ms',
    label: '门控时效',
    shortHelp: '发送链路去重与防重入的有效期（毫秒）。',
    detailHelp: '用于防止快速连点或重复事件触发同一轮流程，设置过短可能导致重复执行。',
  },
  {
  ...
  },
  {
    ...
    detailHelp: '任一关键流失败、schema 非法或语法校验失败都会停止本轮生成且不提交变更。',
    isAdvanced: true,
  },
  {
    key: 'flow.enabled',
    label: '流开关',
    shortHelp: '仅启用的流会参与调度。',
    detailHelp: '关闭后该流配置会保留，但不会请求外部 API，也不会参与合并。',
  },
  {
    key: 'flow.name',
    label: '名称',
    shortHelp: '仅用于你在界面里识别这条流。',
    detailHelp: '建议使用有语义的名称，例如“剧情抽取流”“关系更新流”。',
  },
  {
    key: 'flow.id',
    label: '流ID',
    shortHelp: '请求体中的 flow.id，需在本配置中唯一。',
    detailHelp: '返回体会带 flow_id 与其对应。重复 ID 会导致调试与合并定位困难。',
  },
  {
    key: 'flow.api_url',
    label: 'API URL',
    shortHelp: '该流对应的外部工作流接口地址。',
  ...
  },
  {
    ...
    detailHelp: '仅在你的服务端要求时填写。前端会以密码框显示，但仍建议使用受限密钥。',
  },
  {
    key: 'flow.priority',
    label: '优先级',
    shortHelp: '用于多流结果合并冲突决策。',
    detailHelp: '按优先级降序合并；同优先级按流顺序后者覆盖前者。',
  },
  {
    key: 'flow.timeout_ms',
    label: '超时',
    shortHelp: '单条流请求最大耗时（毫秒）。',
    detailHelp: '超时会导致该流失败。失败策略固定为中止发送，因此请设置合理值。',
  },
  {
    key: 'flow.context_turns',
    label: '上下文楼层数',
    shortHelp: '发送给该流的历史消息楼层数量。',
    detailHelp: '数值越大上下文更完整，但 payload 也更大。建议按模型上下文成本调优。',
  },
  {
    key: 'flow.headers_json',
    label: '额外请求头',
  ...
  },
  {
    ...
    detailHelp: '每条规则都包含 start/end，提取结果会进入 flow request 的 context 构建流程。',
  },
  {
