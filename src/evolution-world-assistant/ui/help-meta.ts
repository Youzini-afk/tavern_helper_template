...
const FIELD_HELP_LIST: FieldHelpMeta[] = [
  ...
  },
  {
    ...
    label: 'Dyn 写入模式',
    shortHelp: '控制 Dyn 条目是直接覆盖，还是按 Markdown 列表做只增/增减。',
    detailHelp:
      '覆盖模式会直接采用 AI 返回正文。只增模式会把返回列表去重追加到现有条目。增减模式会把返回列表视为目标集合，对现有列表做增删对齐。',
  },
  {
    key: 'flow.dyn_write.activation_mode',
    label: 'Dyn 激活模式',
    shortHelp: '决定 Dyn 是仅作为控制器仓库，还是直接参与酒馆世界书激活。',
    detailHelp:
      '控制器仓库模式下 Dyn 会保持红灯，真正注入仍由 Controller getwi() 负责。直接世界书激活模式下，Dyn 会启用并按下面的世界书参数参与匹配。',
  },
  {
    key: 'flow.dyn_write.profile.comment',
    label: 'Dyn 注释',
    shortHelp: '写入到世界书条目的 comment 字段。',
    detailHelp: '可用于标记条目用途、来源工作流或人工备注，便于在世界书中排查。 ',
  },
  {
    key: 'flow.dyn_write.profile.strategy.type',
    label: '策略类型',
    shortHelp: '对应酒馆世界书的触发策略类型。',
    detailHelp: '常见如 constant、selective 等。这里直接写入条目元数据，不额外做语义限制。',
  },
  {
    key: 'flow.dyn_write.profile.strategy.scan_depth',
    label: '扫描深度',
    shortHelp: '控制条目按什么深度策略参与扫描。',
    detailHelp: '默认 same_as_global，表示沿用酒馆全局扫描深度。需要特殊行为时可在这里单独指定。',
  },
  {
    key: 'flow.dyn_write.profile.probability',
    label: '概率',
    shortHelp: '世界书条目的命中概率百分比。',
    detailHelp: '只在直接世界书激活模式下参与实际激活判断。控制器仓库模式下会保留为元数据，不直接生效。',
  },
  {
    key: 'flow.dyn_write.profile.position.type',
    label: '位置类型',
    shortHelp: '控制条目注入到提示词链路中的位置类型。',
    detailHelp: '例如 before_character_definition。这里保持为原始世界书字段，方便与你当前酒馆配置对齐。',
  },
  {
    key: 'flow.dyn_write.profile.position.role',
    label: '注入角色',
    shortHelp: '对应世界书注入时使用的消息角色。',
    detailHelp: '常见为 system，也可按需要改成 user 或 assistant。',
  },
  {
    key: 'flow.dyn_write.profile.position.depth',
    label: '注入深度',
    shortHelp: '控制 in-chat 注入时的深度层级。',
    detailHelp: '数值越大通常表示越靠后或更深层的插入位置，具体以酒馆世界书实现为准。',
  },
  {
    key: 'flow.dyn_write.profile.position.order',
    label: '注入顺序',
    shortHelp: '同位置下多个世界书条目的排序值。',
    detailHelp: '数值用于细化同位置条目的先后顺序，便于稳定控制多个 Dyn 的注入先后。',
  },
  {
    key: 'flow.dyn_write.profile.strategy.keys',
    label: '主关键词',
    shortHelp: '世界书条目的主触发关键词列表。',
    detailHelp: '支持逗号或换行输入。保存时会归一化为字符串数组。',
  },
  {
    key: 'flow.dyn_write.profile.strategy.keys_secondary.keys',
    label: '次关键词',
    shortHelp: '世界书条目的次级关键词列表。',
    detailHelp: '用于更细粒度的触发条件，与主关键词一起构成更复杂的激活规则。',
  },
  {
    key: 'flow.dyn_write.profile.strategy.keys_secondary.logic',
    label: '次关键词逻辑',
    shortHelp: '控制次关键词按 and/not 等哪种逻辑组合。',
    detailHelp: '例如 and_any、and_all、not_any、not_all。该字段会原样写回世界书配置。',
  },
  {
    key: 'flow.dyn_write.profile.extra.group',
    label: '分组',
    shortHelp: '设置条目所属的世界书分组。',
    detailHelp: '可用于与酒馆的 group/group scoring 机制配合，让同类 Dyn 在激活时彼此联动或互斥。',
  },
  {
    key: 'flow.dyn_write.profile.extra.groupWeight',
    label: '分组权重',
    shortHelp: '控制分组评分时的权重值。',
    detailHelp: '只在启用分组评分的配置下实际参与分值计算；否则仅作为元数据保存。',
  },
  {
    key: 'flow.dyn_write.profile.effect.sticky',
    label: 'Sticky',
    shortHelp: '条目命中后持续保留的轮数。',
    detailHelp: '留空表示不设置。只在世界书直接激活模式下参与实际注入行为。',
  },
  {
    key: 'flow.dyn_write.profile.effect.cooldown',
    label: 'Cooldown',
    shortHelp: '条目命中后进入冷却的轮数。',
    detailHelp: '留空表示不设置。用于控制命中后多少轮内不再重复触发。',
  },
  {
    key: 'flow.dyn_write.profile.effect.delay',
    label: 'Delay',
    shortHelp: '条目满足条件后延迟多少轮再生效。',
    detailHelp: '留空表示不设置。适合需要滞后介入的 Dyn 条目。',
  },
  {
    key: 'flow.generation',
    label: '生成参数',
